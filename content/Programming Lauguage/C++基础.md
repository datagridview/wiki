...
注意⚠️：不能用一个数组初始化另一个数组，也不能直接赋值。

##### 复杂的数组声明
```C++
int *ptr[10]; //ptrs是含有10个整型指针的数组
int (*ptr)[10] = &arr; //ptrs指向含有10个整数的数组
int &ref[10] = /* @#$ */ //不存在的
...
注意⚠️：这两个函数不是成员函数，而是要将数组作为参数。

```C++
int i[] = {1,2,3};
int *beg = begin(i); //首元素指针
int *last = end(i); //尾元素下一个位置
```
